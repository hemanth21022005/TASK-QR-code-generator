...
function buildQRString() {
  switch (currentType) {
    ...
    case 'wifi': {
      ...
      // WiFi QR format: WIFI:T:<enc>;S:<ssid>;P:<pass>;H:<hidden>;;
...
btnGenerate.addEventListener('click', () => {
  ...
  try {
    ...
    showError('Could not generate QR code. The data may be too long. Try reducing the input or lowering error correction.');
    console.error(e);
  }
});
...
btnSVG.addEventListener('click', () => {
  ...
  let rects = '';
  const cellSize = 1; // 1px per module pixel; SVG scales cleanly

  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const idx = (y * size + x) * 4;
      const r = data[idx], g = data[idx + 1], b = data[idx + 2];
      // If pixel is close to foreground colour, draw a rect
      const fgR = parseInt(fg.slice(1, 3), 16);
      const fgG = parseInt(fg.slice(3, 5), 16);
      const fgB = parseInt(fg.slice(5, 7), 16);
      const dist = Math.abs(r - fgR) + Math.abs(g - fgG) + Math.abs(b - fgB);
      if (dist < 80) {
